namespace ClickableTransparentOverlay
{
    ...
    unsafe internal sealed class ImGuiRenderer : IDisposable
    {
        ...
        internal ImGuiRenderer(ID3D11Device device, ID3D11DeviceContext deviceContext, int width, int height)
        {
            this.device = device;
        ...
        uint i = 60;
        internal void Update(float deltaTime, Action DoRender)
        {
            var io = ImGui.GetIO();
        ...
        internal IntPtr CreateImageTexture(Image<Rgba32> image, Format format)
        {
        ...
        internal bool RemoveImageTexture(IntPtr handle)
        {
            using var tex = this.DeRegisterTexture(handle);
            return tex != null;
        }

        internal void UpdateFontTexture(string fontPathName, float fontSize, ushort[]? fontCustomGlyphRange, FontGlyphRangeType fontLanguage)
        {
            var io = ImGui.GetIO();
            ...
            if (fontCustomGlyphRange == null)
            {
                switch (fontLanguage)
                {
                    ...
                    case FontGlyphRangeType.Cyrillic:
                        ...
                        break;
                    default:
                        throw new Exception("Font Glyph Range is not supported.");
                }
            }
            else
            {
        ...
        {
            internal int ScissorRectsCount = 0, ViewportsCount = 0;
            internal RawRect[] ScissorRects = new RawRect[16];
            internal Viewport[] Viewports = new Viewport[16];
            internal ID3D11RasterizerState RS = default;
            internal ID3D11BlendState BlendState = default;
            internal Color4 BlendFactor = default;
            internal int SampleMask = 0;
            internal int StencilRef = 0;
            internal ID3D11DepthStencilState DepthStencilState = default;
            internal ID3D11ShaderResourceView[] PSShaderResource = new ID3D11ShaderResourceView[1];
            internal ID3D11SamplerState[] PSSampler = new ID3D11SamplerState[1];
            internal ID3D11PixelShader PS = default;
            internal ID3D11VertexShader VS = default;
            internal ID3D11GeometryShader GS = default;
            internal int PSInstancesCount = 256, VSInstancesCount = 256, GSInstancesCount = 256;
            internal ID3D11ClassInstance[] PSInstances = new ID3D11ClassInstance[256];
            internal ID3D11ClassInstance[] VSInstances = new ID3D11ClassInstance[256];
            internal ID3D11ClassInstance[] GSInstances = new ID3D11ClassInstance[256];
            internal PrimitiveTopology PrimitiveTopology = 0;
            internal ID3D11Buffer IndexBuffer = default;
            internal ID3D11Buffer[] VertexBuffer = new ID3D11Buffer[1], VSConstantBuffer = new ID3D11Buffer[1];
            internal int IndexBufferOffset = 0;
            internal int[] VertexBufferStride = new int[1], VertexBufferOffset = new int[1];
            internal Format IndexBufferFormat = 0;
            internal ID3D11InputLayout InputLayout = default;
