namespace ClickableTransparentOverlay
{
    ...
    /// </summary>
    internal abstract class Overlay : IDisposable
    {
        private readonly string title;
        private readonly Format format;

        private WNDCLASSEX wndClass;
        protected Win32Window window;
        ...
        /// </param>
        internal Overlay(bool DPIAware)
        {
            this.VSync = true;
            this._disposedValue = false;
            this.overlayIsReady = false;
            this.title = T.RandomString(16);
        ...
        internal bool ReplaceFont(string pathName, int size, FontGlyphRangeType language)
        {
        ...
        /// <returns>>true if the font replacement is valid otherwise false.</returns>
        internal bool ReplaceFont(string pathName, int size, ushort[] glyphRange)
        {
            if (!File.Exists(pathName))
            {
                return false;
        ...
        /// </summary>
        internal static int NumberVideoDisplays
        {
            get
            {
        ...
        internal void AddOrGetImagePointer(string filePath, bool srgb, out IntPtr handle, out uint width, out uint height)
        {
        ...
        /// <param name="srgb"> a value indicating whether pixel format is srgb or not.</param>
        /// <param name="handle">output pointer to the image in the graphic device.</param>
        internal void AddOrGetImagePointer(string name, Image<Rgba32> image, bool srgb, out IntPtr handle)
        {
        ...
        internal bool RemoveImage(string key)
        {
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (this.selfPointer != IntPtr.Zero)
            {
                ...
                {
                    throw new Exception("Failed to Unregister class during dispose.");
                }
        ...
        private async Task InitializeResources()
        {
            ...
            {
                throw new Exception("Failed to Register class of name");
            }
